  ...
  return (
    <aside className="w-64 bg-slate-950 text-slate-300 flex flex-col min-h-screen border-r border-slate-800/80 shrink-0 select-none relative z-20 shadow-xl">
      ...
      <nav className="flex-1 px-3 py-3 space-y-1 overflow-y-auto">
        {/* Back Button above Navigation Menu */}
        <div className="pb-2 mb-2 px-1 border-b border-slate-800/60">
          <button
            type="button"
            ...
            className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold transition-all duration-200 group ${
              canGoBack
                ? 'bg-slate-900/90 hover:bg-slate-800 text-slate-200 hover:text-white border border-slate-800 hover:border-slate-700 shadow-sm active:scale-[0.98] cursor-pointer'
                : 'opacity-40 text-slate-500 cursor-not-allowed bg-slate-900/30 border border-slate-900/60'
            }`}
            title={canGoBack ? (previousTab ? `Go back to ${menuItems.find(m => m.id === previousTab)?.label || previousTab}` : 'Go back to previous page') : 'No previous page'}
          >
            <div className="flex items-center gap-2.5">
              <div className={`w-6 h-6 rounded-lg flex items-center justify-center transition-colors ${
                canGoBack
                  ? 'bg-slate-800 group-hover:bg-brand-500/20 text-brand-400 group-hover:text-brand-300'
                  : 'bg-slate-850 text-slate-600'
              }`}>
                <ArrowLeft className={`w-3.5 h-3.5 transition-transform ${canGoBack ? 'group-hover:-translate-x-0.5' : ''}`} />
              </div>
              <span className="tracking-wide">Back</span>
            </div>

            {canGoBack && previousTab && (
              <span className="text-[10px] text-slate-400 font-medium truncate max-w-[85px] bg-slate-800/80 px-2 py-0.5 rounded-md border border-slate-700/50">
                {menuItems.find(m => m.id === previousTab)?.label?.split(' ')[0] || previousTab}
              </span>
            )}
          </button>
        </div>
